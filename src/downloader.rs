...
pub fn init_worklist(
    ...
    for (name, value) in dependencies {
        ...
        let dep: DependencyLite = value
            .clone()
            .try_into()
            .context("Failed to parse dependency")?;
        match dep {
            DependencyLite::Simple(version) => {
                worklist.push((name.to_string(), version.to_string()));
            }
            DependencyLite::Complex {
                version,
                package,
                features,
            } => {
                debug!("Skipping features {:?}", features);
                name_to_use = match package {
                    Some(package) => package,
                    None => name_to_use,
                };
                worklist.push((name_to_use, version));
            }
        }
    }
